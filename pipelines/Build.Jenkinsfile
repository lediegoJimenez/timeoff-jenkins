        ...
        stage('Deploy') {
            steps {
                ...
                withCredentials([usernamePassword(credentialsId: 'nexus-admin', passwordVariable: 'NEXUS_PASS', usernameVariable: 'NEXUS_USER')]) {
                    sh "curl -v -u $NEXUS_USER:$NEXUS_PASS --upload-file ${env.WORKSPACE}/artifacts/timeoff-diego-jimenez.tar.gz  http://192.168.1.144:8081/repository/timeoff-raw/org/gorilla/1.0/timeoff-diego-jimenez-1.0.tar.gz"
                }
            }
        }
    }
